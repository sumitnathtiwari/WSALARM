...
#define EMAIL_FROM_BASE64 "bmlsYmhhc2thckBuaWwuY29t"
#define EMAIL_PASSWORD_BASE64 "bmlsMTIzNDU="
...
soundDetectedVal = digitalRead (soundDetectedPin) ; // read the sound alarm time
 int irs1=analogRead(ir1);
 int irs2=analogRead(ir2);
 /*Serial.print("irs1 value");
 Serial.println(irs1);
 delay(1000);
  Serial.print("irs2 value");
 Serial.println(irs2);
 delay(1000);
  Serial.print("sound value-");
 Serial.println(soundDetectedVal);
 delay(1000);*/
  if ((irs1>500 && irs2>500) && soundDetectedVal==0) // If we hear a sound
  {
    ...
    /*if(!send_flag){ // check if we expect to send an email
     ...
     else if(soundDetectedVal==0 && irs1<500 && irs2<500)
    {
    send_flag = true;
    digitalWrite(Buzzer,HIGH);
    digitalWrite(LED,HIGH);
    ...
    else if(soundDetectedVal==1 &&(irs1>500 && irs2>500))
    {
      digitalWrite(Buzzer,HIGH);
    digitalWrite(LED,HIGH);
    delay(1000);
